#ifndef CHANCE_COMMU_HEADER_H_INCLUDED
#define CHANCE_COMMU_HEADER_H_INCLUDED
...
{
    int num_joueur; //joueur 1 2 3 ou 4 ?
    char nom[TAILLE_NOM];
...
void transaction(t_joueur *j,int montant); //IN: un joueur e tun montant(positif ou negatif), sert a faciliter les transactions

char* corruption[TAILLE_LISTE]; //cartes chance
char* comite_olympique[TAILLE_LISTE]; //cartes commu

void tirer_corruption(t_joueur *j,char* liste_corruption); //tirez une carte chance
void tirer_comite(t_joueur *jA,t_joueur *jB,t_joueur *jC,t_joueur *jD,char* liste_commite,char* liste_corruption); //tirez une carte communauté (celui qui tire la carte est le j)

bool victoire(t_joueur *j1, t_joueur *j2, t_joueur *j3, t_joueur *j4);

#endif // CHANCE_COMMU_HEADER_H_INCLUDED
