...
bool nb_propriete(t_joueur tab_joueur[], t_case plateau[], int i, int j)
/*
i l'indice du joueur qui possede la case et j l'indice de la position de la case
*/
{
    int nb_case = 0;
    int numero = plateau[j].groupe_cartes;
    for(int k = 0 ; k<tab_joueur[i].nb_propriete; k++)
    {
        if(tab_joueur[i].proprietes[k].groupe_cartes == numero)
        {
            nb_case += 1;
        }
    }
    if(numero == 1 || numero == 4 || numero == 5 || numero == 6 || numero == 8)
    {
        if(nb_case == 2)
        {
            return true;
    ...
    else if(numero == 2 || numero == 3 || numero == 7)
    {
    ...
    */
    ...
    if ((plateau[tab_joueur[i].position].num_joueur != tab_joueur[i].numero) && (plateau[tab_joueur[i].position].num_joueur != 0) && (plateau[tab_joueur[i].position].groupe_cartes <= 8))//regarde si la case appartient a quelqu'un d'autre que le joueur et si c'est une propriete
    {
        ...
        switch(option)
        {
            case 0:
            {
                int nb_prop = nb_propriete(tab_joueur, plateau, plateau[tab_joueur[i].position].num_joueur-1, tab_joueur[i].position);
                if(nb_prop == 1)
                {
                    //plateau[tab_joueur[i].position].base_loyer *= 2;
                    if(tab_joueur[i].argent >= plateau[tab_joueur[i].position].base_loyer*2)
                    {
                        tab_joueur[i].argent -= plateau[tab_joueur[i].position].base_loyer*2;
                        tab_joueur[plateau[tab_joueur[i].position].num_joueur-1].argent += plateau[tab_joueur[i].position].base_loyer*2;
                    }/*
                    else
                    {
            ...
            case 1:
            {
                if(nb_propriete(tab_joueur, plateau, plateau[tab_joueur[i].position].num_joueur-1, tab_joueur[i].position) == true)
                {
                    plateau[tab_joueur[i].position].loyer_1_maison *= 2;
            ...
            case 2:
            {
                if(nb_propriete(tab_joueur, plateau, plateau[tab_joueur[i].position].num_joueur-1, tab_joueur[i].position) == true)
                {
                    plateau[tab_joueur[i].position].loyer_2_maison *= 2;
                    if(tab_joueur[i].argent >= plateau[tab_joueur[i].position].loyer_2_maison)
                    {
                        ...
                        tab_joueur[plateau[tab_joueur[i].position].num_joueur-1].argent += plateau[tab_joueur[i].position].loyer_2_maison;
                    }/*
                    else
                    {
                        en_faillite(tab_joueur, i, cases.base_loyer, banque, cases.num_joueur);
            ...
            case 3:
            {
               if(nb_propriete(tab_joueur, plateau, plateau[tab_joueur[i].position].num_joueur-1, tab_joueur[i].position) == true)
                {
                    plateau[tab_joueur[i].position].loyer_3_maison *= 2;
            ...
            case 4:
            {
                if(nb_propriete(tab_joueur, plateau, plateau[tab_joueur[i].position].num_joueur-1, tab_joueur[i].position) == true)
                {
                    plateau[tab_joueur[i].position].loyer_4_maison *= 2;
        ...
        else // le joueur possede soit des maisons soit un hotel
        {
            ...
            if(choix[0] == 49)//le joueur veut vendre
            {
               ...
               do
                ...
                {
                    printf("coucou");
                    printf("    %s  ", tab_joueur[i].proprietes[0].nom);
                    tab_joueur[i].proprietes[choix2[0]-49].nb_maison -= 1;
                }
                if(tab_joueur[i].proprietes[choix2[0]-49].nb_maison > 0)//le joueur possede au moins une maison, il vend donc 1 seul maison
                {
                    printf("    %d  ", tab_joueur[i].proprietes[choix2[0]-49].nb_maison );
                    tab_joueur[i].proprietes[choix2[0]-49].nb_maison -= 1;
                    printf("    %d  ", tab_joueur[i].proprietes[choix2[0]-49].nb_maison );
                    ...
                    case 1:
                        {
                            ...
                            if(option[0] == 49)
                            {
                                tab_joueur[i].proprietes[choix2[0]-49].nb_hotel =0;
                                tab_joueur[i].proprietes[choix2[0]-49].nb_maison =4;
                            ...
                            {
                                tab_joueur[i].proprietes[choix2[0]-49].nb_hotel -=1;
                                tab_joueur[i].proprietes[choix2[0]-49].nb_maison +=3;
                                banque->nb_hotel_restant += 1;
                            ...
                            {
                                tab_joueur[i].proprietes[choix2[0]-49].nb_hotel -=1;
                                tab_joueur[i].proprietes[choix2[0]-49].nb_maison +=2;
                                banque->nb_hotel_restant += 1;
