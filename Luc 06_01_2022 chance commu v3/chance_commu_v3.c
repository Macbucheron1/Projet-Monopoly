...
void afficher_liste(char* liste[],int i) //afficher l'intitulé des cartes
{
    int iterateur=0;
...
int nb_maisons_joueur(t_joueur tab_joueur[],int i)//compter le nombre de maison d'un joueur
{
    int nb_maisons=0;
    for (int j=0; j<tab_joueur[i].nb_propriete; j++)
    {
        nb_maisons+=tab_joueur[i].proprietes[j].nb_maison;
    }
    if (nb_maisons>32)//c'est que le joueur n'a aucune propriete
    {
        nb_maisons=0;
    }
    return nb_maisons;
...
int nb_hotels_joueur(t_joueur tab_joueur[],int i)//compter le nombre d'hotel d'un joueur
{
    int nb_hotels=0;
    for (int j=0; j<tab_joueur[i].nb_propriete; j++)
    {
    ...
    if (nb_hotels>12)//c'est que le joueur n'a aucune propriete
    {
        nb_hotels=0;
    }
    return nb_hotels;
