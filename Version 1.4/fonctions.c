...
void tour(t_joueur* p_joueur, t_case plateau[], t_banque* p_banque) // en cours par Nathan
{
    ...
    else // Le joueur n'est pas en prison
    {
        ...
        acheter_maison_hotel(p_joueur, plateau, p_banque);
...
bool acheter_maison_hotel(t_joueur* p_joueur, t_case plateau[], t_banque* banque) // faire le blindage
{
...
//renvoie un booleen(true si il a achete false sinon)
    if(plateau[p_joueur->position].groupe_cartes <=8)//regarde si c'est bien des propriete
    {
        if (plateau[p_joueur->position].num_joueur == p_joueur->numero)//la case appartient au joueur
        {
            if((plateau[p_joueur->position].nb_maison <= 3) && (plateau[p_joueur->position].nb_hotel ==0))//regarde si il y a 3 maison ou moins et pas d'hotel
            {
                if(banque->nb_maison_restante!= 0)//regarde si il reste des maisons dans la banque
                {
                    if(p_joueur->argent >= plateau[p_joueur->position].val_maison)// regarde si le joueur a assez d'argent
                    {
                        int choix;
                        printf("vous pouvez acheter une maison. voulez-vous en acheter une? oui[1]  non[2]\n");//propose d'acheter une maison
                        scanf("%d", &choix);
                        if(choix == 1)
                        {
                            plateau[p_joueur->position].nb_maison+=1;//ajoute une maison
                            p_joueur->argent-=plateau[p_joueur->position].val_maison;//enleve l'argent au joueur
            ...
            else if((plateau[p_joueur->position].nb_maison == 4) && (plateau[p_joueur->position].nb_hotel == 0))//regarde si il y a 4 maison et aucun hotel
            {
                if(banque->nb_hotel_restant != 0)//regarde si il reste des hotels dans la banque
                {
                    if(p_joueur->argent >= plateau[p_joueur->position].val_hotel)//regarde si le joueur a assez d'argent
                    {
                        int choix;
                        printf("vous pouvez acheter un hotel. voulez-vous acheter un hotel? oui[1]  non[2]\n");
                        scanf("%d", &choix);
                        if(choix == 1)
                        {
                            plateau[p_joueur->position].nb_hotel+=1;//ajoute l'hotel
                            plateau[p_joueur->position].nb_maison = 0;//on retire les maison
                            p_joueur->argent-=plateau[p_joueur->position].val_hotel;//on enleve l'argent au joueur
            ...
            else if(plateau[p_joueur->position].nb_hotel == 1)//le joueur possede 1 hotels
            {
                printf("vous ne pouvez rien faire de plus. vous avez deja un hotels\n");
                return false;
            }
        }
        else if(plateau[p_joueur->position].num_joueur != p_joueur->numero)
        {
